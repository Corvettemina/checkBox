...
request = OrderedMergeRequest()
request.presentations = presentaionsArray
operation_id = slides_api.start_merge_and_save(out_path="MyPresentation.pptx", files=None,  request=request, storage="internal")

while True:
    time.sleep(2)
    operation = slides_api.get_operation_status(operation_id)
    ...
    if operation.status == 'Started':
        ...
        result_path = slides_api.get_operation_result(operation_id)
